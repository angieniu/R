...
There are 643,163 rows, i.e., observations, through the lens of 9 variables, ranging from Precinct, Race, LEG, CC, CG, CounterGroup, Party, CounterType, and SumOfCount. The dataset is about king county elections situation in 2016. Precinct, Race, and CounterGroup, Party, and CounterType have character data type, LEG, CC, CG, and SumOfCount have numeric data type. 
`Precinct` is a city or neighborhood name. 
`Race` might mean the positions candidates are running for.
`Party` might mean the political parties involved in each race.
`CounterType` appears to have a mix of information about candidates and votes.
`SumOfCount` means the counts of votes. 

Some column types might not be sensible. For instance, `LEG` `CC` and `CG` might be some information to be identified, rather than to be calculated. Thus, the data type as character might make more sense than as numeric values.
...
**My Response:** CounterType has mixed information. 'Times Counted' might mean that for the particular race in the precinct (e.g. "Registered Voters", "Times Blank Voted", "Times Counted"). I would go to explore the meaning of 'Times Counted' in the following part through calculation and comparision. 

### SumOfCount

```{r}
king_raw %>% distinct(SumOfCount)
```
...
```{r}
king_races <- king_raw %>%
  filter(Race %in% c("Governon", "Lieutenant Governor", "US President & Vice President"))
```
...
```{r}
library(stringr)
king_races <- king_races %>% mutate(in_seattle = ifelse(str_sub(Precinct, 1, 3) == "SEA ", "Seattle", "Not Seattle"))
```
...
king_races %>% 
  filter(Race == "US President & Vice President" & CounterType %in% c("Times Counted", "Registered Voters")) %>% 
  pivot_wider(names_from=CounterType, values_from = SumOfCount) %>%
  mutate(turnout = `Times Counted`/`Registered Voters`) %>% 
  select(-Party, -LEG, -CC, -CG, -CounterGroup)
```
